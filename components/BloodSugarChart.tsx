...
    const numericValue = typeof value === 'number' ? value : parseFloat(value);
    const safeValue = !isNaN(numericValue) ? numericValue : 100;
    const y = chartHeight - (safeValue / 200) * chartHeight;
...
          <Svg width={chartWidth} height={chartHeight} style={{ overflow: 'visible' }}>
            ...
            {/* 🔥 [공백 제거] Defs 마디 뒤에 오던 공백 줄바꿈과 따옴표 오류 흔적을 완벽 청소 */}
            <Path d={`M 0 ${chartHeight - (target_bloodsugar / 200) * chartHeight} H ${chartWidth}`} stroke="#926897" strokeWidth="1" strokeDasharray="4 2" opacity="0.4" />
            {fillPath ? <Path d={fillPath} fill="url(#fillGradient)" /> : null}
            {actualPath ? <Path d={actualPath} stroke="#926897" strokeWidth="3" fill="none" /> : null}
            {predictedPath ? <Path d={predictedPath} stroke="#926897" strokeWidth="2.5" fill="none" strokeDasharray="5 5" opacity="0.6" /> : null}
            {/* 🔥 [한 줄 축약] 원본 프래그먼트 들여쓰기 탭으로 유실되는 문자 현상을 원천 봉쇄 */}
            {sortedTimeline.map((p) => {
              const { x, y } = getCoords(p.measured_at, p.value);
              const isSelected = selectedPoint?.id === p.id;
              return <Circle key={`circle-${p.id}`} cx={x} cy={y} r={isSelected ? 6 : 4} fill={isSelected ? "#926897" : "white"} stroke="#926897" strokeWidth={2} pointerEvents="none" />;
            })}
          </Svg>
